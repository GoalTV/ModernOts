	...
	elseif msgcontains(msg, 'yes') and talk_state == 1 then
		if getPlayerItemCount(cid, golden_mug) >= 1 then
			if doPlayerRemoveItem(cid, golden_mug, 1) == TRUE then
				...
				doCreateTeleport(1387, to, tp)
				doSendMagicEffect(tp, CONST_ME_TELEPORT)
				doCreatureSay(cid, "You have 30 seconds to enter.", TALKTYPE_ORANGE_1)
				addEvent(function() doRemoveItem(getTileItemById(tp, 1387).uid) end, 30 * 1000)
				talk_state = 0
			end
		else
			npcHandler:say('YOU DO NOT, LIERS MUST DIE!', cid)
