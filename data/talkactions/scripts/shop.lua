function onSay(cid, words, param)
	...
	if getPlayerStorageValue(cid, storage) <= os.time() then
		...
		-- Create the query
		local orderQuery = db.storeQuery("SELECT `id`, `type`, `itemid`, `count` FROM `znote_shop_orders` WHERE `account_id` = " .. accid .. " LIMIT 999;")
			...
			if q_type == 1 then
				...
				local itemweight = getItemWeightById(q_itemid, q_count)
					if playerCap >= itemweight then
						db.storeQuery("DELETE FROM `znote_shop_orders` WHERE `id` = " .. q_id .. ";")
